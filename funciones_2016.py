...
def fix_data(dataframes):
    ...
    for i in range(len(dataframe_conjunto)):
        ...
        if pd.isna(dataframe_conjunto.loc[i, 'date']):
        ...
        if pd.isna(dataframe_conjunto.loc[i, 'quantity']):
            ...
            try:
                ...
                if dataframe_conjunto.loc[i, 'quantity'].lower() == 'one':
                    ...
                    dataframe_conjunto.loc[i, 'quantity'] = 2
    ...
    dataframe_conjunto = dataframe_conjunto.sort_values(by = 'weekdate', ascending = True)
    dataframe_conjunto['weekdate'] = dataframe_conjunto['weekdate'].astype(str)
    for i in range(len(dataframe_conjunto)):
